...
class OutcometypeEntity extends AbstractEntity
{
	public function key()
	{
		switch($this->property) {
			...
			case 'url':
				if (auth()->user()->can('read', $this->record))
					return route('office.outcometype.show', $this->record->id);
				else
					return null;

			case 'search':
				return route('office.outcometype.search');

			case 'create':
				if (auth()->user()->can('create', \App\Outcometype::class))
					return route('office.outcometype.create');
				else
					return null;
	...
	public function fieldOutcomesCount()
	{
		switch($this->property) {
			case 'type':
				return 'numeric';
			...
			case 'value':
				return $this->record->outcomes_count;
		}
	}
}
