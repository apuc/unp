...
class OutcomesubtypeEntity extends AbstractEntity
{
	public function key()
	{
		switch($this->property) {
			...
			case 'url':
				if (auth()->user()->can('read', $this->record))
					return route('office.outcomesubtype.show', $this->record->id);
				else
					return null;

			case 'search':
				return route('office.outcomesubtype.search');

			case 'create':
				if (auth()->user()->can('create', \App\Outcomesubtype::class))
					return route('office.outcomesubtype.create');
				else
					return null;
