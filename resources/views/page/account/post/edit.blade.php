...
@section('content')
	<div class="card-wrap account-forecasts-detail">
		...
		<form id="post-form" action="{{ route('account.post.update', ['post_id' => $post->id]) }}" method="post" enctype="multipart/form-data" onsubmit="return false;">
				...
				<div class="col-md-9">
					<select name="sport_id" class="form-control{{ $errors->has('sport_id') ? ' is-invalid' : '' }}" id="sport">
						...
						@foreach ($sports as $sport)
							<option {!! ($sport->id == old('sport_id') || $sport->id == $post->sport_id) ? 'selected="selected"' : '' !!} value="{{ $sport->id }}">{{ $sport->name }}</option>
						@endforeach
					</select>
			...
			<div class="form-group row">
				...
				<div class="col-md-9">
					<select name="tag_id" class="form-control{{ $errors->has('tag_id') ? ' is-invalid' : '' }}" id="tags">
						...
						@foreach ($tags as $tag)
							<option {!! ($tag->id == old('tag_id') || $tag->id == $post->posttags->first()->tag_id ) ? 'selected="selected"' : '' !!} value="{{ $tag->id }}">{{ $tag->name }}</option>
						@endforeach
					</select>
			...
			<div class="btn-account-row">
				@if ($post->poststatus->slug == 'moderation' || $post->poststatus->slug == 'draft')
				...
				@endif
				<span>Сохранено в {{ $post->updated_at->format('H:i') }}</span>
				{{--<a href="#" class="btn btn-link">Предпросмотр</a>--}}
				@if ($post->poststatus->slug == 'draft')
					<a href="javascript: void(0);" onclick="destroyPost('{{ route('account.post.destroy', ['post_id' => $post->id]) }}');" class="btn btn-light mr-0 ml-md-auto"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
				@endif
			</div>
			<script>
				...
				function destroyPost(route, step)
				{
					...
					if (step === 1)
						$.post(route, {
							...
							location.href = '{{ route('account.post.index') }}';
						});
				}
			</script>
		</form>
