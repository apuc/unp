<div class="b-filter-top">
	<div class="b-filter-top__sport-nav">
		<ul class="b-filter-top__sport-main-list">
			<li class="b-filter-top__sport-main-item">
				<a
					...
					class="nav-link {!! false === Briefparameter::get('sport', false) ? 'active' : '' !!}"
				>Все</a>
			</li>
			@if (!is_null(Briefparameter::topical('sport')))
				@if (Briefparameter::get('sport', false))
					<input
						data-ss-pn-parameter="sport"
						type="hidden"
						value="{{ Briefparameter::get('sport') }}"
					>
				@endif

				@foreach (Briefparameter::topical('sport')->getParameters()['values']->slice(0, 3) as $value)
					<li class="b-filter-top__sport-main-item">
						<a
							href="javascript: void(0);"
							data-ss-pn-submit="click"
				...
				@if (Briefparameter::topical('sport')->getParameters()['values']->count() > 3)
					<li class="b-filter-top__sport-main-item b-filter-top__sport-other-button">
						<a class="
							nav-link
							dropdown-toggle

							@if(Briefparameter::topical('sport')->getParameters()['values']->slice(3)->filter(function ($value) {
						...
						<div class="win-nav b-filter-top__sport-other-pulldown">
							<div class="win-cont">
								<ul>
									@foreach (Briefparameter::topical('sport')->getParameters()['values']->slice(3) as $value)
										<li class="nav-item">
											<a
												href="javascript: void(0);"
												data-ss-pn-submit="click"
		...
		@if (!is_null(Briefparameter::topical('day')))
			@if (Briefparameter::get('day', false))
				<input
					data-ss-pn-parameter="day"
					data-ss-filter-input="day"
					type="hidden"
					value="{{ Briefparameter::get('day') }}"
				>
			@endif
			...
			<div class="b-filter-top__calendar b-calendar">
				...
				<div class="b-calendar__date-picker">
					...
					<div class="b-calendar-dates dropdown-menu" aria-labelledby="calendar-dates">
						@foreach (Briefparameter::topical('day')->getParameters()['values'] as $value)
							<div
								data-ss-pn-submit="click"
