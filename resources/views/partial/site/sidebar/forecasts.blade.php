<div data-ss-filter-url="{{ route('site.forecast.filter') }}" class="filter-box accordion" id="filter-box">
	...
	<div class="collapse" id="filter" data-parent="#filter-box">
		...
		@if (!is_null(Forecastparameter::topical('capper')))
				...
				<div class="collapse show" id="capper" data-parent="#filter-options-capper">
					<div id="param-capper" class="filter-options-body">
						...
						>
							...
							@foreach (Forecastparameter::topical('capper')->getParameters()['values'] as $value)
								<option
						...
						@if (Forecastparameter::get('capper', false))
							<input type="hidden" data-ss-pn-parameter="capper" value="{{ Forecastparameter::get('capper') }}">
						@endif
					</div>
				</div>
			</div>
		...
		<div class="filter-btns">
			...
			<a href="{{ route('site.forecast.index') }}" class="btn btn-light">Сбросить фильтр</a>
		</div>
	</div>
</div>
